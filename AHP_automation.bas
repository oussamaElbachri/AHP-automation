...
For p = 1 To i - 2
    ...
    For m = 1 To i - 1 - p
        variable1 = Application.Evaluate(InputBox("enter comparison point between " & ActiveCell.Offset(0, -1).Value & " and " & ActiveCell.Offset(-p, m + p - 1).Value))
...
For o = 0 To i - 2
    ...
    For p = 1 To j - 2
        ActiveCell.Offset(1, 0).Select
        For m = 1 To j - 1 - p
            variable2 = Application.Evaluate(InputBox("enter comparison point between " & ActiveCell.Offset(0, -1).Value & " and " & ActiveCell.Offset(-p, m + p - 1).Value & " for criterion " & Range("A1").Offset(o, 0).Value))
